...
const ResumeDetailPage = () => {
  ...
  const itemDesc = item[`description_${langSuffix}`] || item.description || "No description provided.";
        ...
        <Container maxWidth="md">
          ...
          <Box sx={{ mb: 6 }}>
            <Typography variant="h1" sx={{ 
              fontFamily: "'Anton', sans-serif", textTransform: 'uppercase', color: '#1125d6', 
              textShadow: { xs: '3px 3px 0px #000', md: '5px 5px 0px #000' }, 
              mb: 3, fontSize: { xs: '3.5rem', md: '5.5rem' }, lineHeight: 1 
            }}>
              {itemTitle}
            </Typography>
          ...
          <Box sx={{ 
            bgcolor: '#fff', border: '4px solid #000', boxShadow: '10px 10px 0px #000', p: { xs: 3, md: 5 }, mb: 10, position: 'relative'
          }}>
            ...
            <Typography variant="body1" sx={{ 
              fontFamily: "'Inter', sans-serif", fontSize: { xs: '1.1rem', md: '1.3rem' }, lineHeight: 1.8, fontWeight: 500, whiteSpace: 'pre-wrap', color: '#000'
            }}>
              {itemDesc}
            </Typography>
          </Box>
          ...
          {images.length > 0 && (
            <Box>
              <Typography variant="h2" sx={{ 
                fontFamily: "'Anton', sans-serif", textTransform: 'uppercase', mb: 4, display: 'inline-block', bgcolor: '#000', color: '#fff', px: 2, py: 1, border: '3px solid #000', boxShadow: '6px 6px 0px #f23a18'
              }}>
              ...
              <Grid container spacing={4}>
                {images.map(image => {
                  ...
                  if (imgUrl.includes('/public')) imgUrl = imgUrl.replace('/public', '');
                  const finalImgUrl = imgUrl.startsWith('http') ? imgUrl : `${BACKEND_URL}${imgUrl}`;
                  
                  return (
                    <Grid item xs={12} sm={6} md={4} key={image.id}>
                      <Box
                        sx={{
                          width: '100%', aspectRatio: '4/3', border: '4px solid #000', boxShadow: '8px 8px 0px #000', bgcolor: '#ffed00', transition: 'all 0.2s ease', cursor: 'crosshair',
                          '&:hover': { transform: 'translate(-4px, -4px) rotate(2deg)', boxShadow: '12px 12px 0px #1125d6' }
                        }}
                      >
                        <img src={finalImgUrl} alt="Evidence" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                      </Box>
                    </Grid>
                  );
                })}
